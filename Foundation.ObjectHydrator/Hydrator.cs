...
namespace Foundation.ObjectHydrator
{
    public class Hydrator<T>:IGenerator<T>
    {
        ...
        private void SetPropertyMap<TProperty>(Expression<Func<T, TProperty>> expression, IGenerator<TProperty> generator)
        {
            ...
            PropertyInfo propertyInfo = typeof(T).GetTypeInfo().GetProperty(propertyName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
        ...
        {
            var propertyName = ((MemberExpression)expression.Body).Member.Name;
            PropertyInfo propertyInfo1 = typeof(T).GetTypeInfo().GetProperty(propertyName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
        ...
        private void Populate(object instance)
        {
            ...
            {
                PropertyInfo propertyInfo = instance.GetType().GetTypeInfo().GetProperty(mapping.PropertyName, BindingFlags.Public | BindingFlags.Instance);
        ...
        private void AddTypeMapToPropertyMap()
        {
            ...
            foreach (PropertyInfo propertyInfo in typeOfT.GetTypeInfo().GetProperties())
            {
                if (propertyInfo.CanWrite && !propertyMap.ContainsKey(propertyInfo.Name))
                {
                    ...
                    else if (!propertyInfo.PropertyType.GetTypeInfo().IsInterface)
                    {
